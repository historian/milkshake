module Milkshake
  class Settings
    ...
    def all
      env_keys = ENV.keys.select { |k| k =~ /MILKSHAKE_.*/ }
      keys = @global_config.keys | @local_config.keys | env_keys

      keys.map do |key|
        key.sub(/^MILKSHAKE_/, '').gsub(/__/, ".").downcase
      end
    end
    ...
    # @local_config["MILKSHAKE_PATH"] should be prioritized over ENV["MILKSHAKE_PATH"]
    def path
      path = ENV[key_for(:path)] || @global_config[key_for(:path)]
      return path if path && !@local_config.key?(key_for(:path))

      if path = self[:path]
        "#{path}/#{Milkshake.ruby_scope}"
      else
        Gem.dir
      end
    end
  ...
  private
    ...
    def global_config_file
      file = ENV["MILKSHAKE_CONFIG"] || File.join(Gem.user_home, ".milkshake/config")
      Pathname.new(file)
    end
