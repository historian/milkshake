...
private
  ...
  def install_rails!(path)
    ...
    goto_rails do |rails_root|
      ...
      MilkshakeApp::Template.evaluate('routes.rb'
  ...
  def install_gem!(name)
    ...
    goto_rails do |rails_root|
      ...
      MilkshakeApp::Template.evaluate('root_module.rb',
        options
  ...
  def externalize_data!(shared_path)
    ...
    if self.options.git or shell.yes?('Initialize git? [yN]:')
      Dir.chdir(shared_path.to_s) do
        ...
        MilkshakeApp::Template.evaluate('gitignore_for_data'
        ).write_to('.gitignore')
